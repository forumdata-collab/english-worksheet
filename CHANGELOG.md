...
**根因（實測，唔係估）**：caoni 字形用 `--cg-fs-mult`（JS 由 `CAONI_FS` 設，放大 1.126×）＋基線補償，令大楷／升部墨頂喺**所有** cell size 都越過頂框：
`xs −2.43% · sm −1.63% · md −0.93% · lg −0.42%`（0% = 頂框，負數 = 穿框）。
而 g/p/y 尾已經貼 98.5–98.8%（貼底框係設計要求）→ **唔可以單純下移**（會變穿底框）。

**做法（批量，兩處）**
1. **字級收 1.5%**：`--cg-fs-mult = CAONI_FS × 0.985 / 0.64`（`--cg-mb` 會自動補償基線，所以字級細咗 = 墨頂降 ~1.02%、尾升 ~0.47%）
2. **統一下移 1% cell 高**：新增 CSS var `--cg-shift`（hw-cell = 1%、word-line = 0.75%、sentence-block = 0.5%，同 `--cg-mb` 同一比例）＋ `span.print { position: relative; top: var(--cg-shift) }`（span 係 inline-block，唔影響 layout）
3. ⚠️ 教訓：`--cg-fs-mult` 由 **JS setProperty** 設定 → 改 CSS 嘅 fallback 完全冇效（第一次改錯地方，實測 0 變化）

**實測（改前 → 改後，墨頂 %）**
- xs `−2.43 → +3.13` · sm `−1.63 → +1.78` · md `−0.93 → +1.06` · lg `−0.42 → +2.34`
- h 升部（lg）`−0.48 → +2.28`；e x-height `30.8 → 32.8%`（啱啱落喺草地帶）；g 尾 `98.6–99.2%`（仍然貼底框）
- `audit_overflow.py` 11 情境：零橫向溢出，`worstCapInkTop` 由 **−0.93% 變 +1.06%**（全部入框）
- 回歸 **73/73**

**未受影響**：cursive（Playwrite）唔用 `--cg-fs`／`--cg-shift` → 完全不變。

## 2026-09-20 — v1.16

### Fixed — 句子／詞語過早換行（用戶報「為何 wrap 幾行，明明仍有空間」）＋新增換行基準選項
**根因**：`lineFitCapacity()` 寫死 `Math.round((inner) * 0.88)`。0.88 只在「螢幕 96px 格 + 印刷 2.2cm 格」呢個組合啱，其餘組合全部錯：
- 印刷格改**大**（2.6cm）→ 容量仍係 788，實際列印只放到 ~640 → **印出界**（隱藏 bug）
- 印刷格改**細**（1.5cm）→ 白白蝕 12% 闊度

**修法 ①（公式）**：改用兩個版面換算成同一把尺（每行放幾多字），取較嚴嘅一個：
```
ratio = min(1, (703px × 螢幕格px) / (inner × 印刷格px))
```
（字級 ∝ 格大小，兩邊都有相同嘅 0.64 × fontScale × 天草泥倍數 → 約掉）
實測（inner 896px）：
- md + 2.2cm → **812**（舊 788，+3%）
- md + 2.6cm → **687**（自動收緊，唔再印出界）
- md + 1.5cm → **896**（+13.7%，之前白白蝕）
- 螢幕優先 → 896（全闊）

**修法 ②（選項）**：Layout 加「Sentence wrap 換行基準」：
- `Print 列印優先（安全）`（預設）— 跟實際印刷格大小計，列印唔會超紙邊
- `Screen 螢幕優先（列印可能超邊界）`— 填滿螢幕，適合純螢幕用／自己控制列印比例

**影響**：句子行同詞語行同一條容量，兩種都受益；天草泥三隻字體（Pre 84.8% / PWUSM 84.7%）同樣受益。
