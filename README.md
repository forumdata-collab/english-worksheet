...
├── fonts/              # hand.css + the 5 woff2 actually used: Andika, EduPre, EduDots,
...
- **Regression suite, 48 checks** (headless chromium + real `change` events, results read via `--dump-dom`): all four sections × case × style · 5 guide styles + Sky·Grass·Mud tile aspect · `practice=0` · long-sentence / no-space-word wrapping · cells-per-row font scaling · dictation mode · stroke modal · localStorage · horizontal overflow · JS errors · caoni font switch (both fonts: size fills the cell, family applied, CSS vars written) · dotted tracing · Show-modal font/data switch (caoni+Pre → EduPre letterform + print_pre skeleton). Print layout is tested separately by forcing the `@media print` rules on at A4 width: no overflow.
